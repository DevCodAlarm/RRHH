...
function PermisosContent() {
  ...
  // Filtrar solicitudes por tipo 'permission' y rol
  const getDisplayRequests = () => {
    const baseRequests = requests.filter(req => req.type === "permission")
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    logGlobalActivity({
      ...
      message: `${user.name} solicitó un permiso: ${formTitle}`,
    ...
    logActivity({
      ...
      description: `${user.name} solicitó un permiso: ${formTitle}`,
      status: "pending",
      userId: user.id
    })
  ...
  const handleUpdateStatus = (id: string, newStatus: HRRequest["status"], reqTitle: string, empName: string) => {
    ...
    logGlobalActivity({
      ...
      message: `${newStatus === "approved" ? "Aprobado" : "Rechazado"} permiso para ${empName}`,
      actor: { id: user?.id, name: user?.name, email: user?.email, role: user?.role },
    })

    logActivity({
      type: "solicitud",
      title: `Permiso ${newStatus === "approved" ? "Aprobado" : "Rechazado"}`,
      description: `${newStatus === "approved" ? "Aprobado" : "Rechazado"} permiso para ${empName}`,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "approved":
        return <Badge className="bg-accent/10 text-accent gap-1"><CheckCircle2 className="h-3 w-3" />Aprobado</Badge>
      case "pending":
        return <Badge className="bg-chart-3/10 text-chart-3 gap-1"><Clock className="h-3 w-3" />Pendiente</Badge>
      case "rejected":
        return <Badge className="bg-destructive/10 text-destructive gap-1"><XCircle className="h-3 w-3" />Rechazado</Badge>
      default:
        return null
    }
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-foreground flex items-center gap-2">
            ...
            {isEmployee ? "Mis Permisos" : "Gestión de Permisos"}
          </h1>
          <p className="text-muted-foreground">
            {isEmployee ? "Solicita permisos para asuntos personales o laborales" : "Administra las ausencias y permisos del personal"}
          </p>
        </div>
        {isEmployee && (
            ...
            <DialogContent className="sm:max-w-lg">
              ...
              <form className="space-y-4 mt-4" onSubmit={handleSubmit}>
                <div className="space-y-2">
                  ...
                  <Input id="title" placeholder="Ej: Trámite personal" value={formTitle} onChange={(e) => setFormTitle(e.target.value)} required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="startDate">Fecha del Permiso</Label>
                  <Input id="startDate" type="date" value={formStart} onChange={(e) => setFormStart(e.target.value)} required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="description">Descripción detallada</Label>
                  <Textarea id="description" placeholder="Explica brevemente el motivo..." rows={3} value={formDescription} onChange={(e) => setFormDescription(e.target.value)} />
                </div>
        ...
        <Card className="bg-card border-border shadow-sm">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Aprobados</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border shadow-sm">
      ...
      <Tabs defaultValue="all" onValueChange={setFilter} className="space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <TabsList className="bg-secondary/50 p-1">
            ...
            <TabsTrigger value="approved" className="data-[state=active]:bg-background">Aprobados</TabsTrigger>
            <TabsTrigger value="rejected" className="data-[state=active]:bg-background">Rechazados</TabsTrigger>
          </TabsList>
        ...
        <TabsContent value={filter} className="mt-0">
          <Card className="bg-card border-border">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg font-medium">Listado de Permisos</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {filteredRequests.length === 0 ? (
                  <div className="text-center py-12 border-2 border-dashed border-border rounded-xl">
                    ...
                    <p className="text-muted-foreground">No hay solicitudes de permiso registradas</p>
                  </div>
                ) : (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-4">
                          <div className="w-10 h-10 rounded-full bg-chart-3/10 flex items-center justify-center shrink-0">
                            <span className="text-sm font-bold text-chart-3">{request.avatar}</span>
                          </div>
                          <div className="space-y-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-semibold text-foreground">{request.employeeName}</span>
                              <Badge variant="secondary" className="bg-chart-3/10 text-chart-3 hover:bg-chart-3/20 border-none">
                                Permiso
                              </Badge>
                            </div>
                            ...
                            <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                              <span className="flex items-center gap-1.5 font-medium text-foreground/70">
                                ...
                                Fecha: {new Date(request.startDate).toLocaleDateString('es-DO')}
                              </span>
